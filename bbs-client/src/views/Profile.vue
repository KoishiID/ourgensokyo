...
const rawImage = ref('')        // 原始图片的 dataURL，用于裁剪器
const previewUrl = ref('')      // 裁剪后的预览
const cropperRef = ref(null)    // 裁剪器实例引用
...
/**
 * 加载用户个人资料
 * 根据路由参数中的用户 ID 获取用户信息和帖子列表（支持分页）
 */
async function loadProfile() {
  loading.value = true
  error.value = ''
  try {
    profile.value = await getUserProfile(route.params.id, { page: currentPage.value })
  } catch (e) {
...
/**
 * 读取用户选择的图片文件
 * 将文件转为 dataURL 后赋值给 rawImage，触发裁剪器渲染
 * @param {object} file - el-upload 的文件对象
 */
 ...
 * 裁剪器变更回调
 * 每当裁剪区域变化时更新预览图
 * @param {{ canvas: HTMLCanvasElement }} param - 裁剪器返回的画布对象
 */
function handleCropperChange({ canvas }) {
  if (canvas) {
...
/**
 * 提交裁剪后的头像
 * 将裁剪结果导出为 blob，通过 fetch 直接上传到头像接口，成功后更新本地头像 URL
 */

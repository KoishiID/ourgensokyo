...
graph TB
    ...
    subgraph 安全接入层
        C1[🔒 CloudFlare Access<br/>零信任网关 / 二次验证 / DDoS 防护]
        C2[🌐 Nginx 反向代理<br/>多级路由: / → SPA /wiki → Wiki.js /api → Express]
    end
    ...
    C2 -->|5. /api/* 代理| D
    D -->|6. 中间件链| E
    E -->|7. 路由分发| F & G & H & I & J & K
    F & G & H & I & J -->|8. Prisma| O
    F & G & H & I & J -.->|9. Redis 缓存| P
    K --> Q
    M -->|HTTP 推送| D
    M -->|WebSocket| N
    N --> B
...
| 2 | CF Access 二次验证（邮箱验证码 / 一次性密码） |
| 3 | 通过后，Nginx 分发：静态资源 → Vue SPA，`/wiki/*` → Wiki.js，`/api/*` → Express |
| 4 | Vue SPA 渲染后，Axios 请求 `/api/*` 回 Nginx |
| 5 | Nginx 反向代理到 Express，过中间件链：Rate Limit → CORS → JSON → JWT |
| 6 | 控制器处理业务逻辑，Prisma 操作 PostgreSQL，Redis 缓存热数据 |
